...
export const createListService = (
  ...
  return new Promise((resolve, reject) => {
    ...
    getBoardSnap(boardId, userId)
      .then((boardSnap: FirebaseFirestore.DocumentSnapshot) => {
        ...
        listsCollection
          ...
          .then((querySnap: FirebaseFirestore.QuerySnapshot) => {
            if (querySnap.empty) {
              ...
              newListRef
                ...
                .catch((err) => {
                  ...
                  const rejectResponse = {
                    status: "internal",
                    message:
                      "List was not created. There is a problem with the database. Please try again later.",
                  };
                  reject(rejectResponse);
                  return;
                });
              ...
              console.error(
                "More than one list with nextListId == '' in board " + boardId
              );
              const rejectResponse = {
                status: "internal",
                message:
