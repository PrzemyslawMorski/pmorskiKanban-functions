...
export const moveTaskService = (
  request: IMoveTaskRequest,
  userId: string
): Promise<IMoveTaskResponse> => {
  return new Promise((resolve, reject) => {
    ...
    if (
      (request.targetListId === request.listId &&
        request.srcPrevTaskId === request.targetPrevTaskId &&
        request.srcNextTaskId !== request.targetNextTaskId) ||
      (request.targetListId === request.listId &&
        request.srcPrevTaskId !== request.targetPrevTaskId &&
        request.srcNextTaskId === request.targetNextTaskId)
    ) {
      const rejectResponse = {
        status: "invalid-argument",
        message:
          "The database is out of sync with your view. Please refresh the page to update your view of the board.",
      };
      reject(rejectResponse);
      return;
    }
    ...
    // srcs are equal
    if (
      request.srcNextTaskId !== "" &&
      request.srcPrevTaskId !== "" &&
      request.srcPrevTaskId === request.srcNextTaskId
    ) {
      const rejectResponse = {
        status: "invalid-argument",
        message:
    ...
    // targets are equal
    if (
      request.targetPrevTaskId !== "" &&
      request.targetNextTaskId !== "" &&
      request.targetPrevTaskId === request.targetNextTaskId
    ) {
      const rejectResponse = {
        status: "invalid-argument",
        message:
      ...
      .then((boardSnap) => {
        ...
        if (request.listId === request.targetListId) {
          getListSnap(boardSnap, request.listId)
            .then((srcListSnap) => {
              getPrevCurrentNextTaskSnaps(srcListSnap, request.taskId)
                .then((prevCurrNext) => {
                  ...
                  if (
                    oldPrevId !== request.srcPrevTaskId ||
                    oldNextId !== request.srcNextTaskId
                  ) {
                    const rejectResponse = {
                      status: "invalid-argument",
                      message:
                  ...
                  // task wasn't moved
                  if (
                    oldPrevId === request.targetPrevTaskId &&
                    oldNextId === request.targetNextTaskId
                  ) {
                  ...
                  getTwoTaskSnaps(
                    ...
                    .then((firstSecond) => {
                      ...
                      const targetPrevId =
                        targetPrev !== null ? targetPrev.id : "";
                      const targetNextId =
                        targetNext !== null ? targetNext.id : "";

                      if (
                        (targetPrev === null && targetNext === null) ||
                        (targetPrev !== null &&
                          targetPrev.data().nextTaskId !== targetNextId) ||
                        (targetNext !== null &&
                          targetNext.data().prevTaskId !== targetPrevId)
                      ) {
                        const rejectResponse = {
                          status: "invalid-argument",
                          message:
                      ...
                      if (oldPrevTask !== null) {
                        batch.update(oldPrevTask.ref, {
                          nextTaskId: oldNextId,
                        });
                      }

                      if (oldNextTask !== null) {
                        batch.update(oldNextTask.ref, {
                          prevTaskId: oldPrevId,
                        });
                      }

                      if (targetPrev !== null) {
                        batch.update(targetPrev.ref, {
                          nextTaskId: request.taskId,
                        });
                      }

                      if (targetNext !== null) {
                        batch.update(targetNext.ref, {
                          prevTaskId: request.taskId,
                        });
                      }

                      batch.update(movedTask.ref, {
                      ...
                      batch
                        ...
                        .catch((err) => {
                          ...
                          const rejectResponse = {
                            status: "internal",
                            message:
                              "Task was not moved. There was an error with the database. Please try again later.",
                          };
                          reject(rejectResponse);
                          return;
                        });
          ...
          getTwoListSnaps(boardSnap, request.listId, request.targetListId)
            .then((firstSecond) => {
                        ...
                        .then((colSnap) => {
                          ...
                          getAttachmentDocsForTask(
                            ...
                            .then((attachmentDocs) => {
                              ...
                              const taskDocMovedToTargetList =
                                targetListSnap.ref
                                  .collection("tasks")
                                  .doc(request.taskId);
                              batch.set(
                                taskDocMovedToTargetList,
                                newMovedTaskData
                              );
