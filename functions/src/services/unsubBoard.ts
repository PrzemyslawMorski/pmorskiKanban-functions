...
export const unsubBoardService = (request: IUnsubBoardRequest, userId: string): Promise<IUnsubBoardResponse> => {
    return new Promise((resolve, reject) => {
        ...
        getBoardSnap(request.boardId, userId).then((boardSnap) => {
            ...
            batch.commit().then(() => {
                ...
                const rejectResponse = {
                    status: 'internal',
                    message: "Board was not unsubbed. There is a problem with the database. Please try again later.",
                };
                reject(rejectResponse);
                return;
            });
