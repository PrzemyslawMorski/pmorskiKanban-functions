...
export const addViewerService = (request: IAddViewerRequest, userId: string): Promise<IAddViewerResponse> => {
    return new Promise((resolve, reject) => {
        ...
        getBoardSnap(request.boardId, userId).then((boardSnap) => {
            ...
            admin.auth().getUser(request.userId).then(() => {
                ...
                if (newViewers.indexOf(request.userId) !== -1) { // already a viewer
                    resolve(response);
                    return;
                }
                ...
                batch.commit().then(() => {
                    ...
                    const rejectResponse = {
                        status: 'internal',
                        message: "Viewer was not added. There is a problem with the database. Please try again later.",
                    };
                    reject(rejectResponse);
                    return;
                });
