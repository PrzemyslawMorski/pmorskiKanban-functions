...
export const changeTaskDescriptionService = (
  boardId: string,
  listId: string,
  taskId: string,
        ...
        getListSnap(boardSnap, listId)
          .then((listSnap) => {
            getTaskSnap(listSnap, taskId)
              .then((taskSnap) => {
                taskSnap.ref
                  .update({ description: newTaskDescription })
                  .then(() => {
                  ...
                  .catch((err) => {
                    ...
                    const rejectResponse = {
                      status: "internal",
                      message:
                        "Task's description was not changed. There is a problem with the database. Please try again later.",
                    };
                    reject(rejectResponse);
                    return;
                  });
