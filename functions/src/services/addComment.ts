...
import {getBoardSnap, getListSnap, getTaskSnap, isOwner, isViewer} from "./dbUtils";
...
export const addCommentService = (request: IAddCommentRequest, userId: string): Promise<IAddCommentResponse> => {
        ...
        getBoardSnap(request.boardId, userId).then((boardSnap) => {
            ...
            getListSnap(boardSnap, request.listId).then((listSnap) => {
                getTaskSnap(listSnap, request.taskId).then(() => {
                    admin.auth().getUser(userId).then((authorRecord) => {
                        const commentDoc = admin.firestore().collection("comments").doc();
                        const commentData = {
                            id: commentDoc.id,
                        ...
                        batch.commit().then(() => {
                            ...
                            const response: IAddCommentResponse = {comment: comment};
                            resolve(response);
                            return;
                        }).catch((err) => {
                            console.error(err);
