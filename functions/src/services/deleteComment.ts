...
export const removeCommentService = (request: IDeleteCommentRequest, userId: string): Promise<IDeleteCommentResponse> => {
                ...
                getTaskSnap(listSnap, request.taskId).then(() => {
                    getCommentSnap(request.boardId, request.listId, request.taskId, request.commentId).then((commentSnap) => {
                        ...
                        batch.commit().then(() => {
                            ...
                            const rejectResponse = {
                                status: "internal",
                                message: "Comment wasn't deleted. There is a problem with the database. Please try again later.",
                            };
                            reject(rejectResponse);
                            return;
                        });
