...
export const moveListService = (
  request: IMoveListRequest,
  userId: string
  ...
  return new Promise((resolve, reject) => {
    ...
    if (
      request.targetPrevListId !== "" &&
      request.targetNextListId !== "" &&
      request.targetPrevListId === request.targetNextListId
    ) {
      const rejectResponse = {
        status: "invalid-argument",
        message:
    ...
    if (
      request.srcPrevListId !== "" &&
      request.srcNextListId !== "" &&
      request.srcPrevListId === request.srcNextListId
    ) {
      const rejectResponse = {
        status: "invalid-argument",
        message:
    ...
    if (
      request.srcPrevListId === request.targetPrevListId &&
      request.srcNextListId === request.targetNextListId
    ) {
    ...
    getBoardSnap(request.boardId, userId)
      .then((boardSnap) => {
        ...
        getPrevCurrentNextListSnaps(boardSnap, request.listId)
          .then((prevCurrNext) => {
            ...
            if (
              oldPrevId !== request.srcPrevListId ||
              oldNextId !== request.srcNextListId
            ) {
              const rejectResponse = {
                status: "invalid-argument",
                message:
            ...
            getTwoListSnaps(
              ...
              .then((firstSecond) => {
                ...
                if (
                  ...
                  (targetPrev !== null &&
                    targetPrev.data().nextListId !== targetNextId) ||
                  (targetNext !== null &&
                    targetNext.data().prevListId !== targetPrevId)
                ) {
                  const rejectResponse = {
                    status: "invalid-argument",
                    message:
                ...
                batch.update(movedList.ref, {
                  prevListId: targetPrevId,
                  nextListId: targetNextId,
                });
                ...
                batch
                  ...
                  .catch((err) => {
                    ...
                    const rejectResponse = {
                      status: "internal",
                      message:
                        "List was not moved. There was an error with the database. Please try again later.",
                    };
                    reject(rejectResponse);
                    return;
                  });
